...
log_file=`"${ticat}" display.quiet : display.plain : display.width 9999 : session.desc "${session_id}" |\
	{ grep '\[serverless.regional.mock.start\]' -A 4 || test $? = 1; } |\
	{ grep 'execute-log' -A 1 || test $? = 1; } |\
	tail -n 1 |\
	awk '{print $1}'`
if [ -z "${log_file}" ]; then
	echo "[:(] can't find log path of serverless-regional-service in session ${session_id}" >&2
	exit 1
fi
echo "tidb-cloud.serverless.regional.logfile=${log_file}" >> "${env_file}"
echo "${log_file}"
